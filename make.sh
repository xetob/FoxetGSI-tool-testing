...
$scriptsdir/dirtolink.sh "$systemdir/system" 2>/dev/null
...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY FoxetGSI TOOL" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
date=`date +%Y%m%d`
outputname="$romtypename-$outputtype-$sourcever-$date-FoxetGSI"
...
sed -i "s/$bdisplay/$displayid2=Built\.with\.FoxetGSI\.Tool/" $systemdir/system/build.prop
