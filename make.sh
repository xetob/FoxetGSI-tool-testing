...
if [ "$outputtype" == "Aonly" ]; then
    if [[ ! $(ls "$systemdir/system/etc/init/" | grep *environ*) ]]; then
        ...
        echo "# AUTOGENERATED FILE BY FoxetGSI TOOLS" > "$systemdir/system/etc/init/init.treble-environ.rc"
...
sed -i "s/$bdisplay/$displayid2=Built\.with\.FoxetGSI\.Tools/" $systemdir/system/build.prop
